...
- (void)internalSetUp
{
    ...
    UILongPressGestureRecognizer *gr = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handlePressGesture:)];
...
- (MCNewCustomLayer*)getItemAtIndex:(NSInteger)index {
    return (MCNewCustomLayer*)[self.containerLayer.sublayers objectAtIndex:index];
}
...
- (void)handlePressGesture:(UILongPressGestureRecognizer *)recognizer
{
    ...
    NSArray *selectedLayers = [self findSelectedLayers:location];
    for (MCNewCustomLayer *layer in selectedLayers)
    {
        if (recognizer.state == UIGestureRecognizerStateBegan) {
            [self customLayeredView:self didTouchMainPathOnLayer:layer];
        }
        else {
            [self customLayeredView:self didReleaseMainPathOnLayer:layer];
        }
    }
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if (hitView == self)
    {
        NSArray *selectedLayers = [self findSelectedLayers:point];
        
        // if the touch happened at a point that is not in any
        // sublayers bounds, then pass the touch to the
        // superview.
...
- (NSArray*)findSelectedLayers:(CGPoint)point
{
    NSArray *layers = [self getAllItems];
    NSMutableArray *selectedLayers = [[NSMutableArray alloc] init];
